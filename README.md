...
This token can be generated in the general settings of the github account under settings/tokens and then needs to be copied into the secret. This token can be generated by every account that is part of the repo and has writing access.
I have now generated the token with the psydig account and this can therefore also be done in the future without relying on my account (reschmutz) - the bot works independently form my personal account. I have not defined any expiration date for it so it works until it is changed.
